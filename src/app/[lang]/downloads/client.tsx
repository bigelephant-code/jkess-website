    ...
    <div className="min-h-screen bg-white">
      <section className="relative bg-gray-900 overflow-hidden">
        <div className="absolute inset-0">
          <img src="/images/downloads-banner-bg.png" alt="" className="w-full h-full object-cover" />
        </div>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 pt-24 pb-12 md:pt-28 md:pb-16 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight mb-4">{t('downloads.title')}</h1>
            <p className="text-gray-400 mt-3 max-w-xl mx-auto">{t('downloads.desc')}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="relative pb-20 z-10">
        <div className="max-w-5xl mx-auto px-6">
          {categories.map((cat) => {
            ...
            return (
              <div key={cat.label} className="mb-8 last:mb-0">
                <button onClick={() => setExpanded({ ...expanded, [cat.label]: !isOpen })}
                  className="w-full flex items-center justify-between mb-4 group">
                  <h2 className="text-lg font-semibold text-gray-700 uppercase tracking-widest">
                    {cat.label} <span className="ml-2 text-xs text-gray-600 font-normal">({cat.files.length} files)</span>
                  </h2>
                ...
                {isOpen && (
                  <div className="space-y-2">
                    {cat.files.map((file, idx) => (
                      <motion.a key={file.name} href={file.url} download
                        ...
                        <span className="flex-1 text-sm text-gray-400 group-hover:text-gray-900 transition-colors min-w-0 truncate">{file.name}</span>
          ...
          <div className="mt-12 text-center">
            <p className="text-xs text-gray-600">All documents are hosted directly on our servers. Need help finding a document?<br />
              <a href="mailto:chinaenergymall@163.com" className="text-[#5b5bff] hover:underline">Contact us</a> and we&apos;ll help you out.
            </p>
          </div>
        </div>
      </section>
