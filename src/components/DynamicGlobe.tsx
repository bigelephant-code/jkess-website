...
interface Shot {
  hubIdx: number; targetIdx: number; progress: number; speed: number
}
...
export default function DynamicGlobe() {
  ...
  useEffect(() => {
    fetch('/data/world-land.json')
      ...
      .catch(() => setLoaded(true)) // fallback: hide outlines
  }, [])
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // ── Draw accurate country borders from GeoJSON data ──
      if (worldData) {
        // Country fill
        c.fillStyle = 'rgba(4, 24, 46, 0.6)'
        worldData.forEach(ring => {
          c.beginPath()
      ...
      if (frame % 10 === 0) {
        ...
        const targetIdx = Math.floor(Math.random() * (ALL_TARGETS.length / 3))
      ...
      for (let si = shots.length - 1; si >= 0; si--) {
        ...
        if (shot.progress >= 1) { shots.splice(si, 1); continue }

        const hub = HUBS[shot.hubIdx]
        const targetIdx = shot.hubIdx * 20 + (shot.targetIdx % 20)
        ...
        const color = colMap[shot.hubIdx]
