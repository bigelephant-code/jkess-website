...
export default function NavbarMegaMenu() {
  ...
  const languageHref = (code: LangCode) =>
    code === 'en' ? neutralPath : `/${code}${neutralPath === '/' ? '' : neutralPath}`
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-20 max-w-[1500px] items-center justify-between px-5 lg:px-7">
        ...
        <div className="hidden items-center gap-2 lg:flex xl:gap-4">
          {navigationGroups.map((group) => {
            if (!group.items) {
              return (
                <a key={group.key} href={`${prefix}${group.href}`} className="rounded-xl px-3 py-2 text-sm font-semibold text-white transition hover:bg-white/10 hover:text-green-400">
                  {t(`nav.${group.key}`, group.label)}
                </a>
              )
            }

            const expanded = activeMenu === group.key
            return (
              <button
                key={group.key}
                type="button"
                ...
                onClick={() => setActiveMenu(expanded ? null : group.key)}
                aria-expanded={expanded}
                className={`flex items-center gap-1 rounded-xl px-3 py-2 text-sm font-semibold transition ${expanded ? 'bg-white/10 text-green-400' : 'text-white hover:bg-white/10 hover:text-green-400'}`}
              >
                {t(`nav.${group.key}`, group.label)}
                <motion.span animate={{ rotate: expanded ? 180 : 0 }} transition={{ duration: 0.2 }}><ChevronDown size={15} /></motion.span>
              </button>
            )
          })}
        </div>
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <a href={`${prefix}/shipping-quote`} className="rounded-xl bg-green-500 px-4 py-2.5 text-sm font-bold text-black transition hover:bg-green-400">
        ...
        <div className="flex items-center gap-3 lg:hidden">
          ...
          <button type="button" onClick={() => setMobileOpen((current) => !current)} className="text-white" aria-label="Toggle navigation menu" aria-expanded={mobileOpen}>
              ...
              <div className={`grid gap-2 p-4 ${activeGroup.items.length > 4 ? 'xl:grid-cols-3' : 'md:grid-cols-2'}`}>
                {activeGroup.items.map((item, index) => (
                  <motion.a
                    ...
                    className="group rounded-xl border border-transparent p-4 transition hover:border-green-400/25 hover:bg-white/[0.06]"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="text-sm font-bold text-white group-hover:text-green-400">{item.label}</h3>
                        <p className="mt-2 text-xs leading-5 text-gray-500 group-hover:text-gray-300">{item.description}</p>
                      </div>
      ...
      <AnimatePresence>
        {languageOpen && (
          <motion.div
            initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}
            className="absolute left-1/2 top-[calc(100%+8px)] hidden w-[min(94vw,980px)] -translate-x-1/2 rounded-2xl border border-white/10 bg-[#090909]/[0.98] p-5 shadow-2xl backdrop-blur-2xl lg:block"
          >
            <div className="grid grid-cols-4 gap-2 xl:grid-cols-6">
              {locales.map((locale) => (
                <a key={locale.code} href={languageHref(locale.code)} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition ${lang === locale.code ? 'bg-green-500/15 text-green-400' : 'text-gray-300 hover:bg-white/10 hover:text-white'}`}>
                  <span>{locale.flag}</span><span className="truncate">{locale.name}</span>
                </a>
              ))}
            </div>
          </motion.div>
            ...
            <div className="max-h-[calc(100vh-96px)] overflow-y-auto px-5 py-5">
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                          <div className="space-y-1 pb-3 pl-3">
                            ...
                            {group.items?.map((item) => (
                              <a key={item.href} href={`${prefix}${item.href}`} className="block rounded-xl px-4 py-3 hover:bg-white/[0.06]">
              ...
              <div className="mt-4 border-t border-white/10 pt-5">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {locales.map((locale) => <a key={locale.code} href={languageHref(locale.code)} className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm ${lang === locale.code ? 'bg-green-500/15 text-green-400' : 'text-gray-300 hover:bg-white/10'}`}><span>{locale.flag}</span><span className="truncate">{locale.name}</span></a>)}
                </div>
              </div>
